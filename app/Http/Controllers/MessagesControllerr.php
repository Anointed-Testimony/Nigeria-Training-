...
class MessagesControllerr extends Controller
{
    ...
    //     ], 200);
    // }
    // public function getContactss(Request $request)
    // {
    //     // get all users that received/sent message from/to [Auth user]
    //     $users = Message::join('users',  function ($join) {
    //         $join->on('ch_messages.from_id', '=', 'users.id')
    ...
    // }
        public function getContacts(Request $request)
        {
            ...
            ->select([
                ...
                DB::raw('MAX(users.created_at) as max_created_at'),
            ])
            ->orderBy('max_created_at', 'desc')
            ->groupBy('users.id','users.name','users.user_type','users.email')
            ->paginate($request->per_page ?? $this->perPage);
    ...
    //     })
    //     ->where('users.id','!=',Auth::user()->id)
    //     ->select('users.id', 'users.user_type', ..., DB::raw('MAX(ch_messages.created_at) as max_created_at'))
    //     ->orderBy('max_created_at', 'desc')
    //     ->groupBy('users.id')
    //     ->paginate($request->per_page ?? $this->perPage);
